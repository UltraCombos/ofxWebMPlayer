static char const* g_cstr_frag_shader601 = R"(
#version 150
...
{   
	...
	vec3 rgb;
	rgb.x = clamp(dot(vec3(1.1644, 0.0, 	1.5960), 	ycbcr), 0.0, 1.0);
	rgb.y = clamp(dot(vec3(1.1644, -0.8130, -0.3918), 	ycbcr), 0.0, 1.0);
	rgb.z = clamp(dot(vec3(1.1644, 2.0172, 	0.0), 		ycbcr), 0.0, 1.0);
